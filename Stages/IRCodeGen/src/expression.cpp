...
void expression_H(ASTNode *node, std::string inh_whereToSendString, std::string &varName, TypeExpression &type, VALUE_TYPE &valueType, SPACE &valueSpace){
    ...
    A_PTree node->addAttribute("🔻 type = " + toString(type));                      // 🌳 Adding inh_attr
    A_PTree node->addAttribute("🔻 valueType = " + toString(valueType));            // 🌳 Adding inh_attr
    A_PTree node->addAttribute("🔻 valueSpace = " + toString(valueSpace));          // 🌳 Adding inh_attr
    ...
    {
        expression_H(node->children[0], inh_whereToSendString, varName, type, valueType, valueSpace);
        primary_expression_H(node->children[2], inh_whereToSendString, varName, type, valueType, valueSpace);
    }
}
...
void primary_expression_H(ASTNode *node, std::string inh_whereToSendString, std::string &varName, TypeExpression &type, VALUE_TYPE &valueType, SPACE &valueSpace)
{
    ...
    {
        // Prepare syn_data to be fetched ⏫
        std::string varName1;
    ...
    A_PTree node->addAttribute("⏫ varName = " + varName);                 // 🌴 Adding syn_attr
    A_PTree node->addAttribute("⏫ type = " + toString(type));             // 🌴 Adding syn_attr
    A_PTree node->addAttribute("⏫ valueType = " + toString(valueType));   // 🌴 Adding syn_attr
    A_PTree node->addAttribute("⏫ valueSpace = " + toString(valueSpace)); // 🌴 Adding syn_attr
}
...
//----------------------------------------
    ...
    else if (whichProduction == P3 || whichProduction == P3)
    {
    ...
    else if (whichProduction == P4 || whichProduction == P5)
    {
...
//------------------------------ Space 🚀Change 🔖IR Code of varName1
        ...
        if(valueSpace1 != reqSpace)
        {
            ...
            CODE_BASE.addTAC(valNew, RIGHT_STAR, varName1, NO_ARG);
            varName1 = valNew; // Change the name to the address
            valueSpace1 = SPACE::ADDRESS_SPACE; // Change the space to address space
        }
        else
        {
            varName1 = varName1;
...
//----------------------------------------
    ...
    else if (whichProduction == P6 || whichProduction == P7)
    {
        CERR << "postfix_expression_H - Pointer Member Selection" << std::endl;
        // This is Inc or Dec
...
void shift_expression_H(ASTNode *node, std::string inh_whereToSendString, std::string &varName, TypeExpression &type, VALUE_TYPE &valueType, SPACE &valueSpace)
{
    ...
    std::string P2 = "shift_expression PLUS additive_expression";
    std::string P3 = "shift_expression MINUS additive_expression";
